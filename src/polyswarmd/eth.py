...
def post_transactions():
    ...
    if not g.user and app.config['POLYSWARMD'].require_api_key:
        ...
        try:
            check_withdrawal(body['transactions'])
...
def check_withdrawal(transactions):
    ...
    if len(transactions) != 1:
        ...
        raise ValueError('multiple transactions requires an api-key')

    function_hash = g.chain.w3.toHex(tx.data[:4])
    if len(tx.data) != 68 or function_hash != transfer_signature_hash:
        raise ValueError('calling something other than transfer requires an api-key')
    ...
    if g.chain.nectar_token.address != to or tx.value != 0 or tx.network_id != app.config["POLYSWARMD"].chains['side'].chain_id:
        raise ValueError('sending a transaction to a contract other than the side nectar token requires an api-key')
    if target != g.chain.erc20_relay.address or amount <= 0:
        raise ValueError('transfering NCT to an address other than erc20relay contract requires an api-key')
